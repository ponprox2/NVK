...
import { getDetailPackageAPI, getDetailPackageAPI2, getFreePickUpShiperAPI, getFreeShiperAPI, updateShopOrderPickup } from '../services/index';
...
function DetailOrder() {
  ...
  const handleAssignment = async (idShipper) => {
    ...
    try {
      ...
      console.log(res);
      if (res?.status === 200) {
        navigate('/dashboard/shopOrdersPickUp');
        setOpenToast(true);
        setSeverity('success');
        setError1(res.data);
        setReCall(!reCall);
    ...
    <>
      <Box>
        ...
        <Box style={{ width: '90%', margin: '0 auto', display: 'flex', justifyContent: 'space-between' }}>
          <Box width="90%">
            <h2 style={{ lineHeight: '30px' }}>Chọn Nhân Viên Nhận Đơn Lấy Hàng</h2>
            <Divider />
            ...
            <Typography style={{ lineHeight: '25px' }}>Địa chỉ lấy hàng : {orderDetail?.deliveryAddress}</Typography>
          </Box>
        </Box>
