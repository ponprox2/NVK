...
import { getWorkingTerritory, getRegion, updateRetrieveOrder, getRetrieveOrders, getTerritoryAPI2 } from '../services/index';
...
const TABLE_HEAD = [
  ...
  export default function User() {
    ...
    const [retrieveType, setRetrieveType] = useState(0);
    ...
    const updateAPI = async () => {
      ...
      try {
        const res = await updateRetrieveOrder(body);
        if (res?.status === 200) {
          setOpenToast(true);
          setSeverity('success');
          setError1(res.data);
    ...
    const getShopOrdersAPI = async (body) => {
      try {
        const res = await getRetrieveOrders(body);
    ...
    }, [warehouseStaffID, territoryID, regionsChoose, statusAllChoose, shopName, orderID, retrieveType, reCall]);
      ...
      <Page title="Thu Hồi Đơn Hàng">
        <Container>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '30px' }}>
            <Typography style={{ marginTop: '10px' }}>Loại đơn hàng</Typography>
            <FormControl style={{ marginTop: '10px' , marginLeft: '30px' }}>
              <Select
                ...
                style={{ height: '30px' }}
                value={retrieveType}
                onChange={(e) => setRetrieveType(e?.target?.value)}
              >
                <MenuItem value={0}>Tất cả</MenuItem>
                <MenuItem value={1}>Đơn Lấy Hàng</MenuItem>
                <MenuItem value={2}>Đơn Giao Hàng</MenuItem>
                <MenuItem value={3}>Đơn Trả Hàng</MenuItem>
              </Select>
            </FormControl>
          </Box>
        ...
        <ConfirmDlg
          ...
          onConfirm={updateAPI}
        >
          Bạn có chắc sẽ thu hồi đơn hàng này?
        </ConfirmDlg>
      </Page>
    );
  }
